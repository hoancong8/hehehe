...
for (const line of lines) {
  if (line.trim().startsWith('{"step_index":32')) {
    ...
    // content is a string containing literal `"` and `\n` characters.
    // It's technically a JSON encoded string, so we can parse it.
    try {
        content = JSON.parse(content);
        fs.writeFileSync('c:/Users/admin/OneDrive/Desktop/web_hp_v_2/hehehe/src/systems/SpriteSystem.js', content);
        console.log('Restored SpriteSystem.js properly using JSON.parse!');
    } catch(e) {
